...
from constants import SUFFIX_NLTK, SUFFIX_SPACY, SUFFIX_AUTOPHRASE, SUFFIX_DBPEDIA
...
def filter_span_data_by_grammar(data, texts):
    ...
    for doc_ind, (line, spans) in tqdm(enumerate(zip(texts, data)), total=len(texts)):
        ...
        try:
            ...
            pos_list = [token[1] for token in doc]
            assert len(tokens) == len(pos_list)
            new_spans = []
            for span in spans:
                if span['ed'] - span['st'] == 1:
                    # 筛选掉单个的词
                    continue
                if pos_list[span['ed']-1] not in ['NN', 'NNS', 'NNP', 'NNPS',
                                                  'VB', 'VBD', 'VBG', 'VBN',
                                                  'VBP', 'VBZ']:
                    # 筛选掉一部分词汇，如果结尾不是名词或者现在分词
                    log.info('doc_ind:{}, skipping {}'.format(doc_ind, span))
                    continue
                else:
                    ind = span['st']
                    while ind < span['ed']:
                        if pos_list[ind] not in ['JJ', 'JJR', 'JJS', 'NN', 'NNS', 'NNP', 'NNPS',
                                                 'VBG', 'RB', 'RBR', 'RBS', 'VB', 'VBD', 'VBG',
                                                 'VBN', 'VBP', 'VBZ']:
                            ind += 1
                        else:
                            break
...
def main():
    ...
    # tokenized_text = '../my_sample_data/sample_text.txt'
    ...
    for path, method_name in zip([autophraseFile, dbpediaFile],
                                 ['autophrase', 'dbpedia']):
